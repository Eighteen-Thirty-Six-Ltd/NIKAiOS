...
//-------------------------------------------------------------------------------------------------------------------------------------------------
class GroupView: UIViewController, UITableViewDataSource, UITableViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, SelectUsersDelegate {
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	func myInit(groupId groupId_: String) {
	...
	@objc func loadGroup() {
		...
		DownloadManager.image(link: dbgroup.picture) { path, error, network in
			if (error == nil) {
				self.imageGroup.image = UIImage(contentsOfFile: path!)
			}
		}
	...
	//---------------------------------------------------------------------------------------------------------------------------------------------
	func saveGroupPicture(picture: String) {

		Group.updatePicture(groupId: groupId, picture: picture) { error in
			if (error != nil) {
				ProgressHUD.showError("Network error.")
			}
		}
	...
	func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey: Any]) {
		...
		if let image = info[.editedImage] as? UIImage {
			...
			if let data = squared.jpegData(compressionQuality: 0.6) {
				...
				UploadManager.upload(data: data, name: "group", ext: "jpg", progress: { progress in
					hud.progress = progress
				}, completion: { link, error in
					hud.hide(animated: true)
					if (error == nil) {
						DownloadManager.saveImage(data: data, link: link!)
						self.saveGroupPicture(picture: link!)
					} else {
						ProgressHUD.showError("Network error.")
					}
				})
