...
class GroupsView: UIViewController, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate {
	...
	@objc func loadGroups() {
		...
		var predicate = NSPredicate(format: "members CONTAINS[c] %@ AND isDeleted == NO", FUser.currentId())

		if let text = searchBar.text {
			if (text.count != 0) {
				predicate = NSPredicate(format: "members CONTAINS[c] %@ AND isDeleted == NO AND name CONTAINS[c] %@", FUser.currentId(), text)
			}
		}
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		let chatGroupView = ChatGroupView()
		chatGroupView.myInit(groupId: dbgroup.objectId)
